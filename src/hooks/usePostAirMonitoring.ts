...
import {
  air_monitoring_fetch_response_data_type,
  AirMonitoring_data_type,
} from "../types/airMonitoringDataType";
...
const usePostAirMonitoring = () => {
  ...
  const postData = async (
    data: AirMonitoring_data_type
  ): Promise<air_monitoring_fetch_response_data_type> => {
    setIsLoading(true);

    
    try {
      const response =
        await axios.post<air_monitoring_fetch_response_data_type>(
          `${BASE_URL}/air-monitoring/${data.deviceUid}`,
          data,
